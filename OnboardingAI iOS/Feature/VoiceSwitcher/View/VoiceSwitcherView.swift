    ...
    ZStack(alignment: .bottom) {
      VStack {
        ...
        ScrollView {
          ...
          .playbackMode(
            viewModel.isPlaying
              ? .playing(.fromProgress(0, toProgress: 1, loopMode: .playOnce)) : .paused
          )
          .frame(height: 190)
          Text("Find the voice that resonates with you").frame(alignment: .top)
          VoiceItemView(
            voiceItems: viewModel.voiceItems, voiceItemDidTap: viewModel.selectVoiceItem)
        }
      }
      ...
      HStack {
        ...
        NavigationLink(
          destination: OnboardingView(
            voiceId: choosenVoiceItem?.id ?? 0, voiceSampleId: voiceSampleId)
        ) {
          NextButton(isEnabled: choosenVoiceItem?.id ?? 0 > 0)
        }.disabled(choosenVoiceItem?.id ?? 0 == 0)
        Spacer().frame(width: 16)
      }.frame(alignment: .bottom)
...
struct NextButton: View {
  ...
  private let enabledBackgroundColor = LinearGradient(
    gradient: Gradient(colors: [.orange, .yellow]), startPoint: .top, endPoint: .bottom)
  private let disabledBackgroundColor = LinearGradient(
    gradient: Gradient(colors: [.gray, .gray]), startPoint: .top, endPoint: .bottom)

  var body: some View {
    Button(action: {}) {
      Text("Next").font(.title3).fontWeight(.semibold)
    }
