...
extension VoiceSwitcherView {
    ...
    final class ViewModel {
        ...
        func selectVoiceItem(_ voiceItem: VoiceItemIdentifiable) {
            ...
            Task {
                let fileUri = try? await self.repository.fetchVoiceFile(voiceId: voiceItem.id, voiceSampleId: self.voiceSampleId)
                playAudio(fileUri)
            }
        }
        ...
        private func playAudio(_ fileUri: URL? = nil) {
            ...
            do {
                ...
                print("Error playing voice: \(error)")
            }
        }

        private func getVoiceItems() {
