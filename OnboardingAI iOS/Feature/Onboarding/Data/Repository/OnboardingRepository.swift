...
extension OnboardingView {
    final class OnboardingRepository: OnboardingRepositoryProtocol {
        ...
        func getTranscription(_ voiceId: Int, _ voiceSampleId: Int) async -> String? {
            let url = URL(string: "https://static.dailyfriend.ai/conversations/samples/\(voiceId)/\(voiceSampleId)/transcription.txt")!
            do {
        ...
        private func getVoiceCacheFile(_ voiceId: Int, _ voiceSampleId: Int) -> URL? {
            ...
            let destination = cacheDirectory.appendingPathComponent("voices").appendingPathComponent("\(voiceId)_\(voiceSampleId).mp3")
